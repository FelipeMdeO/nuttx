...
.. note::
   ...
   If ``CONFIG_SMF_INITIAL_TRANSITION`` is not set, ``smf_set_initial()`` and ``smf_set_state()`` function
...
.. note::
   If ``CONFIG_SMF_INITIAL_TRANSITION`` is not enabled, or if the initial state of a parent state is not
...
- Avoid malformed hierarchical state machines by ensuring the state always transitions to a leaf state when
  ``CONFIG_SMF_INITIAL_TRANSITION`` is not enabled, or when a parent state's initial state is undefined.
